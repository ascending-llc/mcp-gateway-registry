    ...
    async def resolve_user_id(self, user_info: dict) -> str | None:
        ...
        try:
            ...
            if username:
                ...
                if user:
                    logger.debug(
                        f"✓ Resolved user_id from MongoDB by username: {user.id} for username: {username}"
                    )
                    return str(user.id)
            ...
            if email:
                ...
                if user:
                    logger.debug(
                        f"✓ Resolved user_id from MongoDB by email: {user.id} for email: {email}"
                    )
                    return str(user.id)
                logger.warning(
                    f"User not found in MongoDB for username: {username}, email: {email}"
                )
