...
async def authorize_redirect(request: Request):
    ...
    api_prefix = (
        settings.auth_server_api_prefix.rstrip("/") if settings.auth_server_api_prefix else ""
    )
    ...
    redirect_url = (
        f"{actual_auth_endpoint}?{query_params}" if query_params else actual_auth_endpoint
    )

    logger.info(f"Redirecting /authorize to {redirect_url} for provider: {auth_provider}")
    return RedirectResponse(
        url=redirect_url, status_code=307
    )  # 307 preserves GET method and query parameters
