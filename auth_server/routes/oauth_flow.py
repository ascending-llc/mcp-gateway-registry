...
class ClientRegistrationRequest(BaseModel):
    ...
    client_uri: Optional[str] = Field(None)
    redirect_uris: Optional[List[str]] = Field(None)
    grant_types: Optional[List[str]] = Field(default=["authorization_code", "urn:ietf:params:oauth:grant-type:device_code"])
    response_types: Optional[List[str]] = Field(default=["code"])
    scope: Optional[str] = Field(None)
    contacts: Optional[List[str]] = Field(None)
    token_endpoint_auth_method: Optional[str] = Field(default="client_secret_post")
...
async def register_client(registration: ClientRegistrationRequest, request: Request) -> ClientRegistrationResponse:
    try:
        ...
        client_metadata = {
            ...
            "grant_types": registration.grant_types or ["authorization_code", "urn:ietf:params:oauth:grant-type:device_code"],
...
async def device_authorization(req: Request, client_id: str = Form(...), scope: Optional[str] = Form(None), resource: Optional[str] = Form(None)):
...
async def device_token(grant_type: str = Form(...), device_code: str = Form(None), client_id: str = Form(...), code: str = Form(None), code_verifier: str = Form(None), refresh_token: str = Form(None), redirect_uri: str = Form(None)):
    ...
    if grant_type == "authorization_code":
        ...
        if code_challenge:
            ...
            if method == "S256":
                computed = _base64.urlsafe_b64encode(hashlib.sha256(code_verifier.encode()).digest()).decode().rstrip("=")
            else:
        ...
        refresh_tokens_storage[rt] = {"client_id": client_id, "user_info": user_info, "scope": token_payload["scope"], "expires_at": refresh_expires_at}

        del authorization_codes_storage[code]

        return DeviceTokenResponse(access_token=access_token, token_type="Bearer", expires_in=3600, scope=token_payload["scope"], refresh_token=rt)
        ...
        if device_data["status"] == "approved" and device_data["token"]:
            return DeviceTokenResponse(access_token=device_data["token"], token_type="Bearer", expires_in=3600, scope=device_data["scope"])
        ...
        for provider_name, config in OAUTH2_CONFIG.get("providers", {}).items():
            if config.get("enabled", False):
                if auth_provider_env and provider_name != auth_provider_env:
                    continue
                enabled.append({"name": provider_name, "display_name": config.get("display_name", provider_name.title())})
...
async def oauth2_login(provider: str, request: Request, redirect_uri: str = None, client_id: str = None, code_challenge: str = None, code_challenge_method: str = None, response_type: str = None, resource: str = None, state: str = None):
    try:
        ...
        response.set_cookie(key="oauth2_temp_session", value=temp_session, max_age=settings.oauth_session_ttl_seconds, httponly=True, samesite="lax")
        return response
...
async def oauth2_callback(provider: str, request: Request, code: str = None, state: str = None, error: str = None, oauth2_temp_session: str = Cookie(None)):
    try:
        if error:
...
async def exchange_code_for_token(provider: str, code: str, provider_config: dict, auth_server_url: str = None) -> dict:
    ...
    async with __import__('httpx').AsyncClient() as client:
        token_data = {"grant_type": provider_config["grant_type"], "client_id": provider_config["client_id"], "client_secret": provider_config["client_secret"], "code": code, "redirect_uri": redirect_uri}
