...
def oauth_error_response(
    error: str, error_description: str = None, status_code: int = 400
) -> JSONResponse:
    content = {"error": error}
...
async def register_client(
    registration: ClientRegistrationRequest, request: Request
    ...
    try:
        ...
        client_metadata = {
            ...
            or "mcp-servers-unrestricted/read mcp-servers-unrestricted/execute",
        ...
        logger.info(
            f"Registered new OAuth client: client_id={client_id}, name={client_metadata['client_name']}"
        )
...
def cleanup_expired_device_codes():
    ...
    expired_codes = [
        code for code, data in device_codes_storage.items() if current_time > data["expires_at"]
    ]
...
def cleanup_expired_authorization_codes():
    ...
    expired_codes = [
        code
        for code, data in authorization_codes_storage.items()
...
async def device_authorization(
    ...
    if not auth_server_url:
        ...
        scheme = (
            "https"
            if req.headers.get("x-forwarded-proto") == "https" or req.url.scheme == "https"
            else "http"
        )
    ...
    logger.info(
        f"Generated device code for client_id: {client_id}, user_code: {user_code}, resource: {resource}"
    )
...
async def device_token(
    ...
    if grant_type == "authorization_code":
        ...
        user_scopes = (
            map_groups_to_scopes(user_groups, settings.scopes_config)
            if user_groups
    ...
    if grant_type == "urn:ietf:params:oauth:grant-type:device_code":
        ...
        if device_data["status"] == "pending":
            return oauth_error_response(
                "authorization_pending", "user has not yet authorized this request"
            )
    ...
    return oauth_error_response(
        "unsupported_grant_type", f"grant_type '{grant_type}' is not supported"
    )
    ...
    try:
        ...
        internal_state = (
            base64.urlsafe_b64encode(
                json := jwt.utils.force_bytes(json := __import__("json").dumps(internal_state_data))
            )
            .decode()
            .rstrip("=")
        )
        ...
        session_data = {
            ...
            or settings.oauth2_config.get("registry", {}).get("success_redirect", "/"),
        }
...
async def oauth2_callback(
    ...
    try:
        ...
        try:
            temp_session_data = signer.loads(
                oauth2_temp_session, max_age=settings.oauth_session_ttl_seconds
            )
                ...
                try:
                    ...
                    if len(path_parts) >= 1:
                        ...
                        resource_metadata_url = (
                            f"{base}/.well-known/oauth-protected-resource/{'/'.join(path_parts)}"
                        )
                        www_authenticate_parts.append(
                ...
                if "id_token" in token_data:
                    ...
                    try:
                        access_claims = jwt.decode(
                            token_data.get("access_token"), options={"verify_signature": False}
                        )
                        mapped_user = {
        ...
        client_redirect_uri = (
            temp_session_data.get("client_redirect_uri") or f"{settings.registry_url}/redirect"
        )
...
async def oauth2_logout(provider: str, request: Request, redirect_uri: str = None):
    try:
        ...
        if not logout_url:
            redirect_url = redirect_uri or settings.oauth2_config.get("registry", {}).get(
