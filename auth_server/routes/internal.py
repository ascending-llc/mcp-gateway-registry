...
async def generate_user_token(request: GenerateTokenRequest):
    try:
        ...
        if not check_rate_limit(username):
            raise HTTPException(status_code=429, detail=f"Rate limit exceeded. Maximum {MAX_TOKENS_PER_USER_PER_HOUR} tokens per hour.")

        expires_in_hours = request.expires_in_hours
        if expires_in_hours <= 0 or expires_in_hours > settings.max_token_lifetime_hours:
            raise HTTPException(status_code=400, detail=f"Invalid expiration time. Must be between 1 and {settings.max_token_lifetime_hours} hours.")
        ...
        if not validate_scope_subset(user_scopes, requested_scopes):
            ...
            raise HTTPException(status_code=403, detail=f"Requested scopes exceed user permissions. Invalid scopes: {list(invalid_scopes)}")
