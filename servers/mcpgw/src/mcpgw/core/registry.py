...
def _generate_service_jwt(user_context: dict[str, Any]) -> str:
    ...
    # Extract subject from user_context
    subject = user_context.get("sub") or user_context.get("username", "")

    # Build extra claims from user context, overriding with mcpgw-specific metadata
    extra_claims = {
        **user_context,  # Spread all user context fields (preserves user_id, groups, scope, etc.)
    ...
    payload = build_jwt_payload(
        subject=subject,
        issuer=settings.JWT_ISSUER,  # mcpgw is now the issuer
        audience=settings.JWT_AUDIENCE,  # Still targeting mcp-registry
        expires_in_seconds=300,  # 5 minutes (short-lived for proxying)
