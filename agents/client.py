...
# Configure logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s|p%(process)d|%(pathname)s:%(lineno)d|%(levelname)s|%(message)s"
)
...
def load_env_config() -> dict[str, str | None]:
    ...
    """
    env_config = {"client_id": None, "client_secret": None, "region": None, "user_pool_id": None, "server_url": None}
