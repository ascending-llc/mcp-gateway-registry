...
def get_auth_mode_from_args() -> bool:
    ...
    parser.add_argument(
        "--use-session-cookie", action="store_true", help="Use session cookie authentication instead of M2M"
    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--prompt", type=str, default=None, help="Initial prompt to send to the agent")

    # Interactive mode argument
    parser.add_argument(
        "--interactive", "-i", action="store_true", help="Enable interactive mode for multi-turn conversations"
    )
    ...
    parser.add_argument(
        "--mcp-tool-name",
        type=str,
        default=DEFAULT_MCP_TOOL_NAME,
        help=f"Name of the MCP tool to filter and use (default: {DEFAULT_MCP_TOOL_NAME})",
    )

    # Verbose logging argument
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose HTTP debugging output")
    ...
    parser.add_argument("--jwt-token", type=str, help="Use a pre-generated JWT token instead of generating M2M token")
    ...
    parser.add_argument(
        "--access-token", type=str, help="Direct access token override (takes precedence over agent credentials)"
    )
    ...
    parser.add_argument("--scopes", type=str, nargs="*", default=None, help="Optional scopes for the token request")
    ...
    if args.use_session_cookie:
        ...
        if not os.path.exists(cookie_path):
            parser.error(
                f"Session cookie file not found: {cookie_path}\n"
...
async def invoke_mcp_tool(
        ...
        if header_name in [
            ...
            if header_name == "Cookie":
                redacted_headers[header_name] = (
                    f"jarvis_registry_session={redact_sensitive_value(session_cookie if session_cookie else '')}"
                )
    ...
    try:
        ...
        use_sse = (
            supported_transports and "sse" in supported_transports and "streamable_http" not in supported_transports
        )
        ...
        if use_sse:
            ...
            if server_path not in SERVERS_NO_MCP_SUFFIX:
                ...
                logger.info(
                    f"invoke_mcp_tool, Using streamable_http transport without /mcp suffix for {server_name}: {server_url}"
                )

        # Connect to MCP server and execute tool call
        logger.info(
            f"invoke_mcp_tool, Connecting to MCP server using {transport_name}: {server_url}, headers: {redacted_headers}"
        )
...
def print_agent_response(response_dict: dict[str, Any], verbose: bool = False) -> None:
    ...
    logger.debug(
        f"print_agent_response called with verbose={verbose}, response_dict keys: {response_dict.keys() if response_dict else 'None'}"
    )
    if verbose:
        ...
        for message in reversed(response_dict["messages"]):
            ...
            if "AIMessage" in message_type or "ai" in str(type(message)).lower():
                ...
                if content:
                    ...
                    if not verbose:
                        logger.warning(
                            f"AI message found but no content extracted. Message type: {message_type}, Message attrs: {dir(message) if hasattr(message, '__dict__') else 'N/A'}"
                        )
...
async def main():
    ...
    if args.jwt_token:
        ...
        if access_token:
            ...
            if agent_credentials:
                agent_settings.user_pool_id = agent_credentials.get("user_pool_id") or agent_credentials.get(
                    "keycloak_realm"
                )
    ...
    try:
        ...
        client = MultiServerMCPClient(
            {"mcp_registry": {"url": server_url, "transport": "streamable_http", "headers": auth_headers}}
        )
        ...
        logger.info(
            f"All available tools: {[tool.name if hasattr(tool, 'name') else tool.__name__ for tool in all_tools]}"
        )
