...
def _should_ignore_file(filename: str) -> bool:
    ...
    return "readable" in filename.lower()
...
def _create_egress_server_config(
    egress_file: Path, ingress_headers: dict[str, str], registry_url: str, config_type: str = "vscode"
) -> tuple[str, dict]:
    """
...
def _create_noauth_server_config(
    ...
    if config_type == "vscode":
        ...
        server_config = {"type": transport_type, "url": service_url, "disabled": False, "alwaysAllow": []}
        if ingress_headers:
            server_config["headers"] = ingress_headers
...
def _generate_vscode_config(
    has_ingress: bool, ingress_file: Path, egress_files: list[Path], noauth_services: list[dict] = None
) -> bool:
    """
...
def _run_refresh_cycle(buffer_seconds: int = DEFAULT_EXPIRY_BUFFER, force_refresh: bool = False) -> None:
    """
...
""",
    ...
    parser.add_argument("--force", action="store_true", help="Force refresh all tokens regardless of expiration status")

    parser.add_argument("--debug", action="store_true", help="Enable debug logging")

    parser.add_argument(
        "--no-kill", action="store_true", help="Do not kill existing instance (will exit if one is running)"
    )
