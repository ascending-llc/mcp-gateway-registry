...
def _save_egress_tokens(token_data: dict[str, Any], provider: str, mcp_server_name: str | None = None) -> str:
...
def _load_existing_tokens(provider: str = None, mcp_server_name: str | None = None) -> dict[str, Any] | None:
...
def _get_supported_providers() -> list[str]:
    ...
    try:
        ...
        external_providers = [name for name, config in providers.items() if not name.startswith("cognito")]
...
""",
    ...
    parser.add_argument(
        "--config-set", type=int, default=None, help="Specific configuration set number (1-100) to process"
    )
        ...
        for config_num in configs_to_process:
            try:
                ...
                token_data = _run_generic_oauth_flow_for_config(
                    config_num=config_num, provider=provider, force_new=args.force, verbose=args.verbose
                )
