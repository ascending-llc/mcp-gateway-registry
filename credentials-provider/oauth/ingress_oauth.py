...
2. Performs M2M authentication using client_credentials grant (Cognito, Keycloak, or Entra ID)
...
def _validate_environment_variables() -> None:
    ...
    if auth_provider == "keycloak":
        required_vars = ["KEYCLOAK_URL", "KEYCLOAK_REALM", "KEYCLOAK_M2M_CLIENT_ID", "KEYCLOAK_M2M_CLIENT_SECRET"]
    elif auth_provider == "entra":
        required_vars = ["ENTRA_TENANT_ID", "ENTRA_CLIENT_ID", "ENTRA_CLIENT_SECRET"]
    else:  # cognito (default)
        required_vars = ["INGRESS_OAUTH_USER_POOL_ID", "INGRESS_OAUTH_CLIENT_ID", "INGRESS_OAUTH_CLIENT_SECRET"]
...
def _perform_entra_m2m_authentication(tenant_id: str, client_id: str, client_secret: str) -> dict[str, Any]:
...
def _perform_m2m_authentication(client_id: str, client_secret: str, user_pool_id: str, region: str) -> dict[str, Any]:
...
def _save_ingress_tokens(token_data: dict[str, Any]) -> str:
    ...
    try:
        ...
        save_data = {
            ...
            "expires_at_human": time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime(token_data["expires_at"]))
...
""",
    ...
    parser.add_argument(
        "--force", "-f", action="store_true", help="Force new token generation, ignore existing valid tokens"
    )
    ...
    try:
        ...
        if auth_provider == "keycloak":
            ...
            keycloak_url = (
                os.getenv("KEYCLOAK_ADMIN_URL") or os.getenv("KEYCLOAK_EXTERNAL_URL") or os.getenv("KEYCLOAK_URL")
            )
            ...
            token_data = _perform_keycloak_m2m_authentication(
                client_id=client_id, client_secret=client_secret, keycloak_url=keycloak_url, realm=realm
            )
        elif auth_provider == "entra":
            ...
            token_data = _perform_m2m_authentication(
                client_id=client_id, client_secret=client_secret, user_pool_id=user_pool_id, region=region
            )
