...
def _validate_environment_variables() -> None:
    ...
    required_ingress_vars = ["INGRESS_OAUTH_USER_POOL_ID", "INGRESS_OAUTH_CLIENT_ID", "INGRESS_OAUTH_CLIENT_SECRET"]

    required_egress_vars = ["EGRESS_OAUTH_CLIENT_ID", "EGRESS_OAUTH_CLIENT_SECRET", "EGRESS_OAUTH_REDIRECT_URI"]
...
class OAuthConfig:
    ...
    def exchange_code_for_tokens(self, code: str, pkce_verifier: str | None = None) -> bool:
        ...
        try:
            ...
            response = requests.post(self.provider_config["token_url"], data=payload, headers=headers, timeout=30)
    ...
    def _save_tokens_to_file(self, token_data: dict) -> None:
        ...
        try:
            ...
            readable_data = {
                ...
                "refresh_token": self.refresh_token,
                "expires_at": self.expires_at,
                "expires_at_human": time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime(self.expires_at))
                if self.expires_at
                else None,
...
class CallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed_path.path != "/callback":
            ...
            logger.warning(f"CallbackHandler: Received request for unexpected path: {parsed_path.path}")
            return
        ...
        if "code" in params:
            ...
            if oauth_config_global:
                try:
                    ...
                    if success:
                        message = (
                            "Authorization successful! Tokens have been saved securely. You can close this window now."
                        )
                    ...
                    message = (
                        "Authorization received but token exchange encountered an error. Check the logs for details."
                    )
...
def run_m2m_flow(config: OAuthConfig) -> bool:
    ...
    try:
        ...
        response = requests.post(config.provider_config["token_url"], data=payload, headers=headers, timeout=30)
...
Supported providers: """
    ...
    parser.add_argument("--redirect-uri", help="OAuth Redirect URI (default: http://localhost:8080/callback)")
    ...
    parser.add_argument(
        "--interactive", "-i", action="store_true", help="Force interactive mode even if some args are provided"
    )
    parser.add_argument("--force", "-f", action="store_true", help="Force new OAuth flow by deleting existing tokens")
    ...
    if provider and provider.startswith("cognito"):
        ...
        client_secret = (
            args.client_secret or config_data.get("client_secret") or os.getenv("INGRESS_OAUTH_CLIENT_SECRET")
        )
        ...
        client_secret = (
            args.client_secret or config_data.get("client_secret") or os.getenv("EGRESS_OAUTH_CLIENT_SECRET")
        )
