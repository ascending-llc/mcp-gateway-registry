        ...
        for agent_name in agent_configs:
            ...
            try:
                if self.generate_token_for_agent(
                    agent_name, keycloak_url=keycloak_url, realm=realm, oauth_tokens_dir=oauth_tokens_dir
                ):
                    success_count += 1
                else:
...
Examples:
  ...
  python generate_tokens.py --all-agents --keycloak-url http://localhost:8080
    ...
    parser.add_argument(
        "--all-agents", action="store_true", help="Generate tokens for all agents found in .oauth-tokens directory"
    )
    parser.add_argument("--client-id", type=str, help="OAuth2 client ID (overrides config file)")
    parser.add_argument("--client-secret", type=str, help="OAuth2 client secret (overrides config file)")
    parser.add_argument("--keycloak-url", type=str, help="Keycloak server URL (overrides config file)")
    parser.add_argument("--realm", type=str, default="mcp-gateway", help="Keycloak realm (default: mcp-gateway)")
    parser.add_argument("--oauth-dir", type=str, help="OAuth tokens directory (default: ../../.oauth-tokens)")
