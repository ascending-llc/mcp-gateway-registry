    ...
    def _get_relevant_documents(self, query: str, *, run_manager: CallbackManagerForRetrieverRun) -> list[Document]:
        try:
            if self.search_type == SearchType.NEAR_TEXT:
                return self.adapter.near_text(query=query, collection_name=self.collection_name, **self.search_kwargs)
            elif self.search_type == SearchType.BM25:
                return self.adapter.bm25_search(query=query, collection_name=self.collection_name, **self.search_kwargs)
