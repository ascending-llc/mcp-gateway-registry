...
class Settings(BaseSettings):
    ...
    MAX_CHUNK_SIZE: int | None = Field(
        default=2048, description="Maximum size of text chunks for vectorization"
    )
    CHUNK_OVERLAP: int | None = Field(
        default=200, description="Overlap size between consecutive chunks"
    )

    # ========== Vector Store Configuration ==========
    VECTOR_STORE_TYPE: str | None = Field(
        default="weaviate", description="Vector database type (supported: weaviate)"
    )
    ...
    WEAVIATE_HOST: str | None = Field(
        default="127.0.0.1", description="Weaviate server host address"
    )
    WEAVIATE_PORT: str | None = Field(default="8080", description="Weaviate server port")
    WEAVIATE_API_KEY: str | None = Field(
        default="", description="Weaviate API key for authentication (optional)"
    )
    ...
    MONGODB_USERNAME: str | None = Field(
        default="", description="MongoDB username for authentication (optional)"
    )
    MONGODB_PASSWORD: str | None = Field(
        default="", description="MongoDB password for authentication (optional)"
    )
