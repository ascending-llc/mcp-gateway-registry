...
def test_setup_metrics_with_otlp_endpoint(
    mock_exporter, mock_reader, mock_metrics, mock_provider, mock_resource
):
    """Verify setup_metrics configures OTLP exporter when endpoint is provided."""
    setup_metrics(
        service_name="test-service", otlp_endpoint="http://localhost:4318", enable_metrics=True
    )
