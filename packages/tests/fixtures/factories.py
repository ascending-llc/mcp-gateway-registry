            ...
            "oauth": factory.Dict(
                {
                    ...
                    "authorization_url": factory.LazyFunction(
                        lambda: f"https://{fake.domain_name()}/authorize"
                    ),
                    "token_url": factory.LazyFunction(
                        lambda: f"https://{fake.domain_name()}/token"
                    ),
                    "scopes": factory.List(
                        [factory.LazyFunction(lambda: fake.word()) for _ in range(2)]
                    ),
                }
            ),
...
class TokenFactory(factory.DictFactory):
    ...
    type = factory.LazyFunction(
        lambda: fake.random_element(elements=["oauth_access", "oauth_refresh", "api_key"])
    )
