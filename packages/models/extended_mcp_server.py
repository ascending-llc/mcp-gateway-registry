...
class ExtendedMCPServer(Document):
    ...
    scope: str = Field(
        default="private_user", description="Access level: shared_app, shared_user, private_user"
    )
    status: str = Field(default="active", description="Operational state: active, inactive, error")
    numTools: int = Field(
        default=0, alias="numTools", description="Number of tools (calculated from toolFunctions)"
    )
    ...
    lastError: datetime | None = Field(
        default=None, alias="lastError", description="Last error timestamp"
    )
    errorMessage: str | None = Field(
        default=None, alias="errorMessage", description="Last error message details"
    )
    ...
    def _create_tool_docs(self, tool_name: str, tool_data: dict) -> list[LangChainDocument]:
        ...
        metadata.update(
            {"tool_name": tool_name, "original_mcp_name": tool_data.get("mcpToolName", tool_name)}
        )
    ...
    def _create_resource_docs(self, resource: dict) -> list[LangChainDocument]:
        ...
        metadata.update(
            {"resource_name": resource.get("name", ""), "resource_uri": resource.get("uri", "")}
        )
    ...
    def _split_if_needed(self, content: str, metadata: dict[str, Any]) -> list[LangChainDocument]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_metadata.update(
                {"chunk_index": i, "total_chunks": len(chunks), "is_chunked": True}
            )
    ...
    def generate_server_content(self) -> str:
        ...
        parts.append(
            f"Contains {num_tools} tools, {num_resources} resources, {num_prompts} prompts"
        )
    ...
    def from_server_info(
        cls, server_info: dict[str, Any], is_enabled: bool = False
    ) -> "ExtendedMCPServer":
        """
