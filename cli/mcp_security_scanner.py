...
def _run_mcp_scanner(
    server_url: str, analyzers: str = DEFAULT_ANALYZERS, api_key: str | None = None, headers: str | None = None
) -> dict:
...
""",
    ...
    parser.add_argument("--api-key", help=f"LLM API key for security scanning (can also use {LLM_API_KEY_ENV} env var)")
    ...
    parser.add_argument(
        "--headers", help='JSON string of headers to include in requests (e.g., \'{"X-Authorization": "token"}\')'
    )
