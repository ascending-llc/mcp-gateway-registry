...
def _make_request(
    ...
    logger.info("  Headers:")
    logger.info(f"    Authorization: Bearer {token[:50]}..." if token else "    Authorization: <NO_TOKEN>")
...
def register_agent(
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logger.error(
                f"  Authorization header: Bearer {token[:50]}..." if token else "  Authorization header: <NO_TOKEN>"
            )
...
""",
    ...
    toggle_parser.add_argument(
        "enabled", type=lambda x: x.lower() == "true", help="Enable (true) or disable (false) the agent"
    )
