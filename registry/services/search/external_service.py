...
class ExternalVectorSearchService(VectorSearchService):
    ...
    async def add_or_update_service(
        ...
        try:
            ...
            server = ExtendedMCPServer.from_server_info(
                server_info=server_info, is_enabled=is_enabled
            )
    ...
    async def search(
        ...
        use_search_type = search_type or self.search_type
        logger.info(
            f"Search: query='{query}', tags={tags}, top_k={top_k}, "
        ...
        try:
            if not query:
                ...
                servers = self.mcp_server_repo.filter(
                    filters=filters, limit=top_k * 2 if tags else top_k
                )
            ...
            logger.info(
                f"Found {len(results)} servers (rerank={'ON' if self.enable_rerank else 'OFF'})"
            )
            return results
    ...
    def _agent_to_server_info(
        self, agent_card_dict: dict[str, Any], entity_path: str
    ) -> dict[str, Any]:
        """
    ...
    async def search_mixed(
        ...
        try:
            ...
            if self.enable_rerank:
                ...
                # Regular search without rerank
                servers = self.mcp_server_repo.search(
                    query=query, search_type=use_search_type, k=search_k
                )
            ...
            for server in servers:
                ...
                relevance_score = (
                    round(server.relevance_score, 4) if hasattr(server, "relevance_score") else 0.0
                )
