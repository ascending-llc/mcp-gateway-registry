...
def _update_config_from_request(config: Dict[str, Any], data: ServerUpdateRequest, server_name: str = None) -> Dict[str, Any]:
    ...
    if 'oauth' in config:
        requires_oauth = config.get('oauth') is not None
...
class ServerServiceV1:
    ...
    async def retrieve_tools_from_server(
        ...
        try:
            ...
            # Build server_info dict for mcp_client
            server_info = {
                "type": decrypted_config.get("type", "streamable-http"),
                "tags": server.tags or [],
            }
            
            # Add headers if present
            if "headers" in decrypted_config:
                server_info["headers"] = decrypted_config["headers"]
            
            # Add apiKey if present (for backend server authentication)
            # Use decrypted config to get the actual key value
            if "apiKey" in decrypted_config:
                server_info["apiKey"] = decrypted_config["apiKey"]
