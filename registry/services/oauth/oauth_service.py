...
class MCPOAuthService:
        ...
        try:
            ...
            if not oauth_config:
                return (
                    None,
                    None,
                    f"Server '{server.serverName}' authentication configuration not found",
                )
    ...
    async def complete_oauth_flow(
        self, flow_id: str, authorization_code: str, state: str
    ) -> tuple[bool, str | None]:
        """
        Complete OAuth flow
    ...
    async def get_valid_access_token(
        ...
        try:
            ...
            logger.info(
                f"Access token expired or missing for {user_id}/{server_name}, attempting refresh"
            )
            ...
            if has_refresh:
                ...
                if success:
                    ...
                    if tokens and tokens.access_token:
                        logger.info(
                            f"Successfully refreshed access token for {user_id}/{server_name}"
                        )
                        return tokens.access_token, None, None
    ...
    async def refresh_token(
        ...
        try:
            ...
            logger.info(
                f"[OAuth] Token refresh successful for {server_id}, "
    ...
    async def validate_and_refresh_tokens(
        self, user_id: str, mcp_server: MCPServerDocument
    ) -> tuple[bool, str | None]:
        """
        ...
        try:
            logger.info(
                f"[OAuth] Validating and refreshing tokens for user={user_id}, server={mcp_server}"
            )
    ...
    async def handle_reinitialize_auth(
        ...
        if access_token_doc is not None:
            ...
            if refresh_valid:
                logger.info(
                    f"[Reinitialize] Access token expired for {server_name}, "
        ...
        if refresh_valid:
            logger.info(
                f"[Reinitialize] No access token for {server_name}, "
    ...
    async def _refresh_and_connect(
        ...
        try:
            ...
            refresh_token_doc, _ = await token_service.get_refresh_token_status(
                user_id, server_name
            )
            if not refresh_token_doc:
                logger.error(
                    f"[Reinitialize] Refresh token disappeared for {server_name}({server_id})"
                )
                return await self._build_oauth_required_response(user_id, server)
            ...
            if success:
                logger.info(
                    f"[Reinitialize] Token refreshed successfully for {server_name}({server_id})"
                )
                return True, self._build_success_response(server)
