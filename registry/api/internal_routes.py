...
async def internal_register_service(
    ...
    if existing_server and not overwrite:
        logger.warning(
            f"INTERNAL REGISTER: Server exists and overwrite=False for path {path}")  # TODO: replace with debug
        return JSONResponse(
    ...
    logger.warning(
        f"INTERNAL REGISTER: Server registered successfully, adding to FAISS index")  # TODO: replace with debug
...
async def internal_remove_service(
    ...
    if not success:
        logger.warning(
            f"INTERNAL REMOVE: Failed to remove service at path '{service_path}'")  # TODO: replace with debug
        return JSONResponse(
            status_code=500,
            content={
...
async def internal_toggle_service(
    ...
    if not success:
        logger.warning(
            f"INTERNAL TOGGLE: Failed to toggle service at path '{service_path}'")  # TODO: replace with debug
        return JSONResponse(
            status_code=500,
            content={
