...
class OAuthReconnectionManager:
    ...
    async def reconnect_servers(self, user_id: str) -> dict[str, bool]:
        ...
        logger.info(
            f"Found {len(servers_to_reconnect)} servers to reconnect: {list(servers_to_reconnect)}"
        )
    ...
    async def try_reconnect_server(
        self, user_id: str, server_id: str, force_new: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            if flow_state.status == "failed" or flow_age_seconds > flow_ttl_seconds:
                ...
                if not was_cancelled:
                    logger.debug(
                        f"OAuth flow failed for {server_id}: "
    ...
    def _cleanup_on_failed_reconnect(self, user_id: str, server_id: str) -> None:
        ...
        try:
            asyncio.create_task(
                self.mcp_service.connection_service.disconnect_user_connection(user_id, server_id)
            )
        except Exception as e:
    ...
    async def _get_user_tokens(self, user_id: str, server_id: str) -> OAuthTokens | None:
        ...
        try:
            ...
            logger.error(
                f"Failed to get user tokens: user={user_id}, server={server_id}, error={e}"
            )
            return None
...
def get_reconnection_manager(
    mcp_service: Any = None, oauth_service: Any = None
    ...
    if _reconnection_manager_instance is None:
        ...
        _reconnection_manager_instance = OAuthReconnectionManager(
            mcp_service=mcp_service, oauth_service=oauth_service
        )
