...
class OAuthHttpClient:
    ...
    def build_authorization_url(
        ...
        if not state.startswith(flow_id):
            logger.warning(
                f"State format issue: state does not start with flow_id. state={state}, flow_id={flow_id}"
            )
    ...
    async def exchange_code_for_tokens(
        ...
        try:
            ...
            if (
                auth_method
                and "client_secret_basic" in auth_method
                and flow_metadata.client_info.client_secret
            ):
    ...
    async def refresh_tokens(
        self, oauth_config: dict[str, Any], refresh_token: str
    ) -> OAuthTokens | None:
