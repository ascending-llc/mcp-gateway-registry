...
class TestProxyJsonRpcRequest:
    ...
    async def test_sse_streaming_response(self, mock_server):
        ...
        mock_backend_response = Mock()
        mock_backend_response.status_code = 200
        mock_backend_response.headers = {"content-type": "text/event-stream", "mcp-session-id": "session-123"}
        mock_backend_response.aiter_bytes = async_iter_bytes
...
class TestSessionManagement:
    ...
    def mock_server(self):
        ...
        server.serverName = "Test Server"
        server.path = "/test"
        server.config = {"url": "http://localhost:8000/mcp", "requiresInit": True, "type": "streamable-http"}
        return server
    ...
    def mock_stateless_server(self):
        ...
        server.config = {"url": "http://localhost:8000/mcp", "requiresInit": False, "type": "streamable-http"}
        return server
    ...
    def tool_request(self):
        ...
        return ToolExecutionRequest(
            server_id="test-server-id", server_path="/test", tool_name="test_tool", arguments={"query": "test"}
        )
    ...
    async def test_execute_tool_with_existing_initialized_session(
        ...
        with (
            patch("registry.api.proxy_routes.server_service_v1.get_server_by_id", return_value=mock_server),
    ...
    async def test_execute_tool_stateless_server_skips_session(
        ...
        with (
            patch("registry.api.proxy_routes.server_service_v1.get_server_by_id", return_value=mock_stateless_server),
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_clears_session_on_non_200_response(self, tool_request, user_context, mock_server):
        ...
        with (
            ...
            patch("registry.api.proxy_routes._build_authenticated_headers", new_callable=AsyncMock) as mock_auth,
            patch(
                "registry.api.proxy_routes._proxy_json_rpc_request", new_callable=AsyncMock, return_value=error_response
            ),
        ):
            mock_auth.return_value = {"Authorization": "Bearer token"}
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_clears_session_on_401_auth_error(self, tool_request, user_context, mock_server):
    ...
    async def test_execute_tool_uses_correct_transport_type(
        ...
        with (
            ...
            patch("registry.api.proxy_routes.get_session", return_value=None),
            patch(
                "registry.api.proxy_routes.initialize_mcp_session", new_callable=AsyncMock, return_value="session-id"
            ) as mock_init,
            patch("registry.api.proxy_routes._build_authenticated_headers", new_callable=AsyncMock) as mock_auth,
            patch(
                "registry.api.proxy_routes._proxy_json_rpc_request",
