...
const StarRatingWidget: React.FC<StarRatingWidgetProps> = ({
  ...
  const renderStars = (_count: number, filled: boolean, size: 'small' | 'large' = 'large') => {
    ...
    return (
      <IconComponent className={`${sizeClass} ${filled ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`} />
    );
  };
          ...
          {showSuccess ? (
            <div className='text-center py-6'>
              ...
              <div className='flex justify-center items-center gap-1 mb-2'>
                ...
                <span className='ml-2 text-sm text-gray-600 dark:text-gray-400'>({selectedRating} stars)</span>
              </div>
              <p className='text-sm text-gray-600 dark:text-gray-400'>New average: {averageRating.toFixed(1)} ★</p>
            </div>
          ) : isSubmitting ? (
            // Loading State
            <div className='text-center py-6'>
              ...
              <p className='text-sm font-medium text-gray-900 dark:text-white'>Submitting your rating...</p>
            </div>
          ) : (
            // Rating Form
            <>
              ...
              {currentUserRating && (
                <p className='text-xs text-gray-500 dark:text-gray-400 mb-3'>Currently: {currentUserRating} stars</p>
              )}

              {/* Star Selection */}
              <div className='flex items-center justify-center gap-2 my-4' role='radiogroup' aria-label='Select rating'>
                {[1, 2, 3, 4, 5].map(star => (
                  <button
                    key={star}
