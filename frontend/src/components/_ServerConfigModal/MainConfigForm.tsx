  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        <div className="h-16 w-16 rounded-full border-2 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center text-gray-400 dark:text-gray-500">
           {formData.icon ? (
             <img src={formData.icon} alt="Icon" className="h-full w-full rounded-full object-cover" />
           ) : (
             <PlusIcon className="h-8 w-8" />
           )}
        </div>
        <div>
          <p className="text-sm font-medium text-gray-900 dark:text-white">Icon (optional)</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">Minimum size 128 × 128 px</p>
        </div>
      </div>
      ...
      <div>
       ...
       <div>
        <label className="block text-sm font-medium text-gray-900 dark:text-gray-100 mb-2">Authentication</label>
        <div className="flex items-center justify-between p-3 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700">
          <span className="text-sm text-gray-900 dark:text-gray-100">
            {formData.authConfig.type === 'auto' ? 'Auto Detect' : 
             formData.authConfig.type === 'api-key' ? 'API Key' : 'Manual OAuth'}
          </span>
          <button
            type="button"
            onClick={onAuthClick}
